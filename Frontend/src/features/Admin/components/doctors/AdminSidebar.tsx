    ...
    <>
      ...
      <aside className="fixed left-0 top-0 z-20 hidden h-screen w-[272px] border-r border-slate-200 bg-[#f4fbfb] text-[#12213a] shadow-[10px_0_35px_rgba(15,118,110,0.08)] lg:flex lg:flex-col">
        ...
        <nav className="flex-1 space-y-3 overflow-y-auto px-4 py-8">
          {mainMenu.map((item) => (
            <SidebarButton
              key={item.label}
              item={item}
              active={item.label === "Dashboard" && activeTab === "all"}
              onClick={item.label === "Dashboard" ? () => setActiveTab("all") : undefined}
            />
          ))}
        ...
        <div className="space-y-3 p-4">
          <div className="flex items-center gap-3 rounded-lg bg-[#dff5f3] p-4">
            <img
            ...
            <div className="min-w-0 flex-1">
              <p className="font-black text-[#12213a]">Admin User</p>
              <p className="truncate text-sm text-[#405169]">admin@petsveta.com</p>
            </div>
            <ChevronDown size={16} />
          </div>
...
const SidebarButton = ({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex w-full items-center gap-4 rounded-lg px-5 py-3.5 text-left font-semibold transition ${
        active
