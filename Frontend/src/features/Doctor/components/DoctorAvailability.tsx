    ...
    <main className="min-h-screen bg-[#F8FAFA] text-[#20263D]">
      ...
      <aside className="fixed left-0 top-0 hidden h-screen w-72 border-r border-slate-200 bg-white lg:block">
        <div className="flex items-center gap-3 border-b border-slate-200 px-7 py-7">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-[#078b91] bg-[#D4E2E0]/50 text-[#078b91]">
            🐾
          </div>
        ...
        <nav className="space-y-2 px-5 py-7">
          <SidebarItem icon={<LayoutDashboard size={21} />} label="Dashboard" />
          ...
          <SidebarItem icon={<Users size={21} />} label="My Patients" />
          ...
          <SidebarItem icon={<DollarSign size={21} />} label="Earnings" />

          <SidebarItem icon={<User size={21} />} label="Profile" />

          <SidebarItem icon={<Settings size={21} />} label="Settings" />
        </nav>

        <button
          type="button"
          className="absolute bottom-8 left-7 flex items-center gap-3 text-lg font-bold text-red-500"
        >
          <LogOut size={22} />
          Logout
        </button>
      ...
      <section className="lg:pl-72">
        {/* Top Navbar */}
        <header className="flex h-20 items-center justify-between border-b border-slate-200 bg-white px-5 lg:px-10">
          <nav className="hidden items-center gap-10 font-bold lg:flex">
            ...
            <button
              type="button"
              className="border-b-2 border-[#078b91] py-7 text-[#078b91]"
            >
              Doctors
            </button>
          ...
          <div className="ml-auto flex items-center gap-4">
            <input
              ...
              className="hidden w-80 rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none transition focus:border-[#078b91] focus:ring-4 focus:ring-[#D4E2E0]/60 md:block"
            />

            <div className="relative">
              🛒
              <span className="absolute -right-2 -top-2 flex h-5 w-5 items-center justify-center rounded-full bg-[#078b91] text-xs font-black text-white">
                2
              </span>
            </div>

            <div className="flex h-11 w-11 items-center justify-center rounded-full bg-[#D4E2E0]">
              👩
            </div>

            <span className="hidden font-bold sm:block">Ayesha</span>
          </div>
        </header>
                    ...
                    <div className="mt-5 flex justify-center">
                      <button
                        ...
                        className={`relative h-8 w-14 rounded-full transition ${
                          item.enabled ? "bg-[#078b91]" : "bg-slate-300"
                        }`}
                      >
                        <span
...
const SidebarItem = ({
  ...
  return (
    <button
      type="button"
      className={`flex w-full items-center gap-4 rounded-2xl px-5 py-4 text-left text-sm font-black transition ${
        active
